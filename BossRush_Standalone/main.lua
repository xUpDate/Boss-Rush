...
function warp(area, room, x, y, layer)
	...
	function res()
					reset_input['A'] = true  joypad.set(reset_input)
					reset_input['B'] = true  joypad.set(reset_input)
					reset_input['Start'] = true  joypad.set(reset_input)
					reset_input['Select'] = true  joypad.set(reset_input)
	end

while true do
-- MouseInput
	mouse = input.getmouse()
if not mouse["Left"] then optionswitch = false end
...
-- Mouse Box Clicks
----- Start
	if mouse["X"] >= 1 and mouse["X"] <= 23 and mouse["Y"] >= 150 and mouse["Y"] <= 158 and mouse["Left"] then
		bossrush = true
		flags = 1
...
----- Options
	...
	if mouse["X"] >= 25 and mouse["X"] <= 55 and mouse["Y"] >= 150 and mouse["Y"] <= 158 and mouse["Left"] 
		and optionswitch == false and options == false then 
			options = true
			optionswitch = true
	elseif mouse["X"] >= 25 and mouse["X"] <= 55 and mouse["Y"] >= 150 and mouse["Y"] <= 158 and mouse["Left"] 
...
-- Options Window
	if options == true then 
		...
		-- Click / Switch Mechanic
			-- Settings
			if mouse["X"] >= 3 and mouse["X"] <= 8 and mouse["Y"] >= 3 and mouse["Y"] <= 8 and mouse["Left"] then
				fullint = true
				vanilla = false
			...
			-- Mode
			--[[if mouse["X"] >= 3 and mouse["X"] <= 8 and mouse["Y"] >= 24 and mouse["Y"] <= 29 and mouse["Left"] then
				normal = true
				full = false
			elseif mouse["X"] >= 3 and mouse["X"] <= 8 and mouse["Y"] >= 31 and mouse["Y"] <= 36 and mouse["Left"] then
				full = true
				normal = false
			end]]
...
-- End / Cancel Mechanics
	...
	if bossrush == true then 
			if not mouse["Left"] then optionswitch = false end
			if mouse["X"] >= 211 and mouse["X"] <= 238 and mouse["Y"] >= 150 and mouse["Y"] <= 158 and mouse["Left"] and optionswitch == false then
				bossrush = false
				res()
				optionswitch = true
			end
		...
		if memory.readbyte(0x020227A8) == 0x51 and memory.readbyte(0x03000BF4) == 0x8B then res() end
	end
